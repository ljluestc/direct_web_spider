# encoding: utf-8
"""
Comprehensive tests for TyDownloader class.
...
class TestTyDownloaderComprehensive:
    """Comprehensive test suite for TyDownloader"""
    ...
    def test_ty_downloader_initialization(self):
        """Test TyDownloader initialization"""
    ...
    def test_ty_downloader_initialization_empty_list(self):
        """Test TyDownloader initialization with empty list"""
    ...
    def test_ty_downloader_initialization_multiple_items(self):
        """Test TyDownloader initialization with multiple items"""
        items = [
    ...
    @patch('requests.get')
    def test_run_http_error_status(self, mock_get):
        ...
        # Verify callback was NOT called due to HTTP error
        callback.assert_not_called()
    ...
    def test_run_multiple_items_mixed_results(self, mock_valid_html, mock_set_utf8_html, mock_get):
        ...
        items = [
            ...
            self._create_mock_item("http://notfound.com")
        ]
        downloader = TyDownloader(items)
        callback = Mock()
    ...
    def test_downloader_inheritance(self):
        """Test that TyDownloader inherits from Downloader"""
    ...
    def test_downloader_string_representation(self):
        """Test string representation of TyDownloader"""
        items = [self._create_mock_item()]
        downloader = TyDownloader(items)
        
        str_repr = str(downloader)
        assert "TyDownloader" in str_repr

    def test_downloader_equality(self):
        """Test equality comparison of TyDownloader instances"""
        items1 = [self._create_mock_item("http://example1.com")]
        items2 = [self._create_mock_item("http://example2.com")]
        
        downloader1 = TyDownloader(items1)
        downloader2 = TyDownloader(items1)
        downloader3 = TyDownloader(items2)
    ...
    def test_downloader_hash(self):
        """Test hashing of TyDownloader instances"""
        items = [self._create_mock_item()]
        downloader = TyDownloader(items)
    ...
    def test_downloader_deepcopy(self):
        ...
        assert copied_downloader.max_workers == downloader.max_workers
    ...
    def test_downloader_memory_usage(self):
        """Test memory usage of TyDownloader"""
        import sys
    ...
    def test_downloader_thread_safety(self):
        """Test thread safety of TyDownloader"""
        import threading
        import time
    ...
    def test_downloader_process_safety(self):
        """Test process safety of TyDownloader"""
    ...
    def test_max_workers_property(self):
        ...
        # Test that max_workers is read-only (can't be changed after init)
        # This is a design choice - max_workers is set during initialization
        assert downloader.max_workers == 20

    @patch('requests.get')
    @patch('spider.encoding.Encoding.set_utf8_html')
    @patch('spider.utils.utils.Utils.valid_html')
    def test_concurrent_execution(self, mock_valid_html, mock_set_utf8_html, mock_get):
        ...
        items = [self._create_mock_item(f"http://example{i}.com") for i in range(50)]
        downloader = TyDownloader(items)
        callback = Mock()
    ...
    @patch('requests.get')
    def test_fetch_method_return_values(self, mock_get):
        """Test _fetch method return values"""
        # Setup mocks
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.content = b"<html>Test content</html>"
        ...
        with patch('spider.encoding.Encoding.set_utf8_html'), \
             patch('spider.utils.utils.Utils.valid_html', return_value=True):
            result = downloader._fetch(item)
            assert result is True
        
        # Test failed fetch due to invalid HTML
        with patch('spider.encoding.Encoding.set_utf8_html'), \
             patch('spider.utils.utils.Utils.valid_html', return_value=False):
            result = downloader._fetch(item)
            assert result is False
        
        # Test failed fetch due to HTTP error
        mock_response.status_code = 404
        with patch('spider.encoding.Encoding.set_utf8_html'):
            result = downloader._fetch(item)
            assert result is False
